/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ProductEntryWhereInput } from "./ProductEntryWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { ProductEntryOrderByInput } from "./ProductEntryOrderByInput";

@ArgsType()
class ProductEntryFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ProductEntryWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => ProductEntryWhereInput, { nullable: true })
  @Type(() => ProductEntryWhereInput)
  where?: ProductEntryWhereInput;

  @ApiProperty({
    required: false,
    type: [ProductEntryOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [ProductEntryOrderByInput], { nullable: true })
  @Type(() => ProductEntryOrderByInput)
  orderBy?: Array<ProductEntryOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ProductEntryFindManyArgs as ProductEntryFindManyArgs };
